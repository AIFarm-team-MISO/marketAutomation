...
│
├── keywordDictionary/                       # 키워드 사전 관련 폴더
│   ├── build_dictionary.py                  # 사전 생성을 위한 스크립트
│   ├── dictionary_loader.py                 # 엑셀 파일을 로드하여 사전으로 변환하는 로직
│   ├── keyword_extractor.py                 # 사전을 통해 키워드를 추출하는 로직
│   └── dictionary.xlsx                      # 키워드 사전 엑셀 파일
│
├── keywordOptimization/                     # 키워드 최적화 관련 폴더
│   ├── ProductInfo .py                      # 
│   ├── naver_api.py                         # 네이버 API 호출 모듈
│   ├── keyword_classifier                   # 
│   ├── keyword_combination.py               # 키워드 조합 관련
│   └── keyword_filter.py                    # 필터링 및 중복 제거 로직
│
├── productNaming/                           # 상품명 가공 관련 폴더
│   ├── name_handler.py                      # 상품명 가공 로직 메인 파일
│   ├── naming_utils.py                      # 가공에 필요한 유틸리티 함수 모음
│   ├── naming_logic.py                      # 가공에 필요한 로직 함수 모음
│   └── duplicate_checker.py                 # 도매처 상품명 중복 체크 로직
│
│
│
