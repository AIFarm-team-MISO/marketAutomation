...
from rotationFile.rotation_excel_edit_util import update_column_to_9999, adjust_column_by_percentage, swap_image_column, clear_image_columns
...
def market_process(first_sheet_data, market_platform, market_name, dome_name):
    ...
    if market_platform == "네이버":
        ...
        # 19금 카테고리 제거
        delete_row, removed_count  = remove_adult_category_rows(first_sheet_data, "카테고리 번호*")

        if dome_name == "도매토피아":
            modify_sellercode = add_prefix_to_column(delete_row, "판매자 관리코드", "GT")
            modify_count = update_column_to_9999(modify_sellercode, "수량*") #수량변경
            ...
            updated_df, modified_count = convert_http_to_https(delete_row, columns_to_update)
            processed_sheet_data = updated_df

        else:
            processed_sheet_data = delete_row
