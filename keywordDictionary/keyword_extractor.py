...
def gpt_extract_main_and_sub_keywords(product_name, dictionary, model="gpt-3.5-turbo"):
    ...
    forbidden_keywords = (
        ...
        settings.FILTER_UNIT_KEYWORDS +
        settings.FILTER_BLAND_KEYWORDS +
        settings.COUPANG_FILTER_KEYWORDS +
        settings.FILTER_11_KEYWORDS
    )
    forbidden_keywords = list(set(forbidden_keywords))

    # 금지 키워드 텍스트 생성
    forbidden_text = ", ".join(forbidden_keywords[:30])  # 너무 많을 경우 일부만
    forbidden_note = f"📛 추가 금지 키워드 목록 (예시):\n{forbidden_text}, ... 등 {len(forbidden_keywords)}개 이상의 금지 키워드가 존재합니다.\n이 키워드는 절대 포함하지 마세요.\n"
    ...
    prompt = (
        ...
        f"추가 작업1:\n"
        f" 작업내용 : '제품군'의 'GPT연관검색어'를 3~10개 생성하세요.\n"
        f" 'GPT연관검색어'는 다음 조건을 만족해야 합니다:\n\n"

        f"[1] 기본 조건:\n"
        f"- 제품군과 매우 밀접한 키워드여야 하며, 실제 사용자가 검색창에 입력할 법한 **구체적이고 간결한 키워드**로 작성해야 합니다.\n"
        f"- 키워드는 명사 혹은 복합명사로만 작성해주세요. (예: '캠핑의자', '미니쓰레기통')\n"

        f"[2] 유의어 포함:\n"
        f"- 제품군을 대체할 수 있는 **의미가 유사한 단어**를 포함합니다. 실사용자의 표현 방식(외래어, 신조어 등)도 포함 가능합니다.\n"
        f"- 예: '물병' → '보틀', '슬리퍼' → '실내화', '보조배터리' → '짐용충전기'\n"

        f"[3] 니치 키워드 포함:\n"
        f"- 니치 키워드는 검색량은 적지만 전환율이 높은 **특정 용도/대상/상황 기반의 키워드**입니다.\n"
        f" - 예: '보조 배터리' → '여행용 보조 배터리', '비상용 충전기', '학생용 배터리'\n"
        f" - '캠핑의자' → '차박 의자', '낚시용 의자', '트렁크 수납용 의자'\n"
        f" - 예: '의자' → '차박의자', '낚시의자', '트렁크보관용의자'\n"
        f" - 2~3개 이상의 니치 키워드를 반드시 포함해주세요.\n"

        f"[4] 금지 조건:\n"
        f"- 브랜드명, 상품 고유명칭, 상표명, 숫자, 추상적인 표현, 마케팅성 키워드(예: '인기', '고급', '추천', '후기')는 사용하지 마세요.\n"
        f"- 예: '나이키', '샤넬', '아이폰15', '최고', '가성비' 등은 금지입니다.\n"

        f"[5] 키워드 형식:\n"
        f"- 모든 키워드는 **공백 없이** 작성해주세요. (예: 'DVD 케이스' ❌ → 'DVD케이스' ✅)\n"
        f"- 출력 형식은 반드시 리스트 형식으로 주세요: ['키워드1', '키워드2', ..., '키워드N']\n\n"

        f"추가작업1 예시:\n"
        f" - 제품군: 독서등\n"
        f"   GPT연관검색어: 군인독서등, 야간독서조명, LED독서등, 밤독서등, 야간스탠드\n"
        f" - 제품군: 클리어북\n"
        f"   GPT연관검색어: 인사관리책, 회사파일, 문서관리노트, 클리어파일\n"
        f" - 제품군: 미니휴지통\n"
        f"   GPT연관검색어: 작은휴지통, 회사휴지통, 책상쓰레기통, 자동차휴지통, 승용차휴지통\n\n"

        f"추가작업1 주의사항:\n"
        f"- 'GPT연관검색어'에는 반드시 **브랜드명(예: 나이키, 샤넬, 프라다)**이나 **상품명, 상표명, 고유명칭**은 포함하지 마세요.\n"
        f"- 검색 SEO 및 니치 키워드 적합성에 맞게 **간결하고 실용적인 키워드**만 작성하세요.\n"
        f"- 비현실적이거나 상품과 관련 없는 단어(예: 전생, 전투, 상상 등) 또는 용도와 무관한 사양은 제외하세요.\n"
        f"- 의미 중복 없이, 유사 의미이거나 확장된 키워드로 다양성을 확보하세요.\n"
        f"- 'GPT연관검색어'는 반드시 공백이 없는 단어로 작성하세요.\n"
        f"- 숫자(예: '3단', '15인치')는 연관검색어에 절대 포함하지 마세요.\n"
        f"- '브랜드키워드' 및 '금지단어(후기, 추천, 가격, 구매, 연예인)'는 반드시 포함시키지 마세요.\n"
        f"{forbidden_note}"
        ...
        # f" - '연관검색어'에 가능한경우 니치키워드를 포함해 2~3개 포함하세요.\n"       
        # f" - '연관검색어'에는 **브랜드명(예: 나이키, 샤넬, 프라다)**이나 특정 제품의 고유명칭, 상품명은 절대로 포함하지 마세요.\n"
        ...
        # f" - '연관검색어' 에는 '붙여쓰기'(예: 관리도서)가 아닌 '띄어쓰기'(예: 관리 도서)로 연관검색어를 만들어 주세요.\n"
        # # f" - 따라서 '연관검색어' 생성 시, '붙여쓰기' 형태로 생성해 주세요.\n"
...
import time
def call_openai_with_retries(product_name, dictionary, retries=20, delay=2):
